...
where $t_{int}$ is the time at which intervention strategies were introduced (around day 123 in this example). The challenge here is that in a stochastic system we require that the event rates are piecewise constant, and thus the continuous exponential decay is hard to capture. Instead we can approximate this by assuming that the rate only changes after each event (in practice we would do this in a much better manner at the cost of additional computational loads).
