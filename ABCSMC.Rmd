...
> **Note**: To speed things up here we will not use the `run()` function, rather we will create the model using `mparseRcpp()`, and then compile it using `compileRcpp()`, before passing the compiled model to the `ABCSMC()` function. The `run()` function does all this internally, but here we will do it in two stages. Also, the `run()` function requires that the parameters and initial states are added as `data.frame` objects (for consistency), whereas `ABCSMC` requires that the function works with `pars`, `data` and `u` as **vectors**, so the use of `run()` within this function would require us to convert vectors to data frames within the `simBS` function, which is inefficient.
