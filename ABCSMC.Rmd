...
names(postMn) <- priors$parnames

## produce ABC reference table
BS_ref <- ABCRef(500, pars = postMn, func = simBSRef, sumNames = c("finalsize", "finaltime", "SB"), 
                 parallel = T, u = iniStates, model = model, tspan = BS$day, B = BS$B, 
                 tstop = max(BS$day) * 2)
summary(BS_ref)
```
