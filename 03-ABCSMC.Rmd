...
The second argument to the `ABCSMC()` function must be a `data.frame` containing information about the prior distributions for the parameters. Here we will let $\beta \sim U(0, 5)$ and $\gamma \sim U(0, 5)$. We can specify this as follows:

`r ifelse(opts_knit$get("rmarkdown.pandoc.to") == "latex", "\\newpage", "")`

```{r, abc-priors}
## set priors
priors <- data.frame(parnames = c("beta", "gamma"), 
                     dist = rep("unif", 2), 
                     stringsAsFactors = F)
priors$p1 <- c(0, 0)
priors$p2 <- c(5, 5)
...
R0gt <- function(beta, gamma) {
    data.frame(R0gt = as.numeric((763 * beta / gamma) > 1))
}
...
Matching to final epidemic size and the date of the final removal only is quite crude. We could also incorporate information regarding the observed $I$ curves. One option would be to generate some summary measure of distance between the simulated and observed curves, something like a sum-of-squared differences e.g.
$$
    S_I = \sum_{t = 1}^T \left(I_t - I^\prime_t\right)^2,
$$
where $I_t$ is the **observed** counts at time $t$, and $I^\prime_t$ are the corresponding **simulated** counts at time $t$, and $T$ is the total number of time points. Therefore if the simulated time-series matches the data *exactly*, then $S_I = 0$, with large values of $S_I$ corresponding to poor matches between the simulated and observed counts.

To calculate $S_I$, we have to re-compile the model with the `tspan` option turned on.
...
simFlu <- function(pars, data, tols, u, model, tspan, I) {
    ...
    ## return vector if match, else return NA
    if(all(abs(c(finalsize, finaltime, SI) - data) <= tols)){
        return(c(finalsize, finaltime, SI))
    } else {
        return(NA)
    }
}
...
To create some initial tolerances, let's append an initial value for `SI` to our original `tols` object:
...
In practice, we cannot derive a measure of $S_I$ from the **observed** data alone, but we know that if the simulations match the observed data exactly, then $S_I = 0$, so we use this as our target, and append to the `sumStat` vector accordingly:

```{r, abc-tols21}
## set up data to match to
sumStat <- c(sumStat, SI = 0)
```
...
post <- ABCSMC(
    ...
    ngen = 3, 
    parallel = T,
    model = model, tspan = flu$day, I = flu$in_bed
)
```
...
```{task}
Add another summary measure corresponding to the sum-of-squared distances between the observed and simulated $R$ curves (assume that the `flu$convalescence` column corresponds to observed $R$ counts). Re-run your ABC-SMC routine. 
```
...
``{r, abc-solfunc}
## set up function to perform simulation and check matching
simFlu <- function(pars, data, tols, u, model, tspan, I, R) {
    ...
    ## return vector if match, else return NA
    if(all(abs(c(finalsize, finaltime, SI, SR) - data) <= tols)){
        return(c(finalsize, finaltime, SI, SR))
    } else {
        return(NA)
    }
}
...
post <- ABCSMC(
    ...
    ngen = 3,
    parallel = T, 
    model = model, tspan = flu$day, I = flu$in_bed, R = flu$convalescent
)
post
summary(post)
``
