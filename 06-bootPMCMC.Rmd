...
Note that other (better) observation processes could also be used, but this choice allows us to produce some reasonable estimates in a reasonable compuattional load. The observed data, $I_t$, is coded as the `Iobs` column in the `flu_dat` data frame---see Section \@ref{data}. To set up the observation process defined above, we define a `data.frame` as follows:
...
Here the variance is `r flu_train`, which is much larger than 3. Hence let's try increasing the number of particles and repeating these steps.
