...
The `data` we will pass to the `PMCMC()` function will be a `data.frame` with the first column corresponding to `t` and the second corresponding to the *observed* $B$ curve. hence,
...
Once thes have been obtained, we can use the corresponding posterior samples to seed a set of forward simulations into the future up to some pre-determined time point. All of this is done within the `predict()` function; we just need to pass it a suitable `PMCMC` object and a `tspan` argument for the time points we wish to predict to.
...
Now let's predict forward up to day 14, based on the posterior distributions at day 3. To speed this up we will take 1,000 posterior samples, which can be obtained by using the `window()` function to remove the first 2,000 iterations as burn-in, and then thin the remaining 8,000 samples by taking every 8^th^ sample. The `predict()` function produces a `SimBIID_runs` object, which we can plot as before.
...
```
The uncertainties up to the blue dashed line are derived from the bootstrap particle filter, whereas the uncertainties going forward are from direct simulations from the model. Since the $B$ and $C$ curves can be compared directly to the observed data, we can add the observed data in as additional arguments to the `plot()` method here, we just have to add an additional `matchData` argument to tell the function which columns of the data to plot against which output from the model. 

> The `matchData = c("Bo = B")` below tells the `plot()` function to match the column called `B0` in the data set to the `B` class from the simulations.
...
for(i in 1:length(pred_days)) {
    ...
    ## run predictions forward in time
    post_pred[[i]] <- predict(post, tspan = (pred_days[i] + 1):14)
    
    ## plot predictions
    post_pred[[i]] <- plot(post_pred[[i]], quant = c(0.6, 0.7, 0.8, 0.9),
        data = BS_dat, matchData = c("Bo = B"))
}
``

``{r, echo = F}
post_pred[[1]] + ggplot2::ggtitle("Predictions at day 5")
``

``{r, echo = F}
post_pred[[2]] + ggplot2::ggtitle("Predictions at day 8")
``

``{r, echo = F}
post_pred[[3]] + ggplot2::ggtitle("Predictions at day 11")
``

We can see that the uncertainties reduce as we fit to more data and as the epidemic begins to die out. We can also see that the actual observed outbreak is within the main uncertainty bounds for each of the model fits, but that the predictions get better the more data we fit to.
